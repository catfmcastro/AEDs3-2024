...
public class Hash {
  ...
  // Ponteiros
  private int ptrGlobal; // ponteiro global
  private int contBuckets; // contagem de buckets escritos no file
  ...
  private long endReg; // endereco do registro
  ...
  public Hash() {
    ...
    try {
      ...
      if (hashBuckets.length() == 0 && hashIndex.length() == 0) { // se os arquivos estiverem vazios
        ...
        hashIndex.writeInt(1); // escreve o ptrGlobal (inicia em 1)
        hashIndex.writeInt(2); // contagem de buckets (inicia em 2)
        ...
        setContBuckets(2); // update contagem de buckets escritos no arquivo
      }
    } catch (Exception e) {
      System.out.println();
      System.err.println(
  ...
  public void readCbAndPg() {
    try {
      hashIndex.seek(0); // posiciona o ponteiro no inicio do arquivo
